...
AZURE_OPENAI_PREVIEW_API_VERSION = os.environ.get("AZURE_OPENAI_PREVIEW_API_VERSION", "2025-01-01-preview")
...
# Temporarily disabled for testing - to bypass On Your Data
SHOULD_USE_DATA = False

# Replace your current init_openai_client with this
def init_openai_client(use_data=SHOULD_USE_DATA):
    try:
        ...
        if use_data:
            # Important: Use the extensions endpoint but with newer api-version
            base_url = f"{str(endpoint).rstrip('/')}/openai/deployments/{AZURE_OPENAI_MODEL}/extensions"
            client = AsyncAzureOpenAI(
                base_url=base_url,
                api_version="2025-01-01-preview",   # Use a newer preview version
...
def prepare_model_args(request_body):
    ...
    # FINAL CLEAN VERSION - ONLY max_completion_tokens
    model_args = {
        "messages": messages,
...
async def send_chat_request(request):
    ...
    # Aggressive removal of max_tokens
    model_args.pop("max_tokens", None)
    if "extra_body" in model_args:
        model_args["extra_body"].pop("max_tokens", None)
...
async def generate_title(conversation_messages):
    ...
    try:
        ...
        response = await azure_openai_client.chat.completions.create(
            ...
            messages=messages,
            temperature=1.0,
            max_completion_tokens=64,     # ONLY this - no max_tokens
        )
        
        content = response.choices[0].message.content.strip()
        title = json.loads(content).get('title', 'New Conversation')
        return title
    except Exception as e:
